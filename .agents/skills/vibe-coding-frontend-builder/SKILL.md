...
### 步驟 4：核心 MVP 建立與增量功能迭代
- **目的**：從 0 到 1 產出 MVP 核心功能，或新增/修改單一業務功能，實施防禦性編程（如 XSS 防護、資料向下相容），並自動同步遠端。
- **Prompt 模板 (MVP 初始建立)**：
  > `幫我打造 [專案名稱] 的核心功能。在表單上加入：[主要欄位與預設值]，卡片/列表需呈現這些資訊，並將資料自動儲存至 LocalStorage`
- **Prompt 模板 (增量功能疊代)**：
  > `幫我在 [目標 UI 位置/元件] 上加入 [新功能/欄位描述]，需求為：[需求細節 1]、[需求細節 2]，其他原有功能不要改動`
